...
proc initializeNativeAsmParser*(): Bool {.inline.} =
  when defined(_NATIVE_ASMPARSER):
    native_Asmparser()
    return 0
  else:
    return 1
...
proc initializeNativeAsmPrinter*(): Bool {.inline.} =
  when defined(_NATIVE_ASMPRINTER):
    native_Asmprinter()
    return 0
  else:
    return 1
...
proc initializeNativeDisassembler*(): Bool {.inline.} =
  when defined(_NATIVE_DISASSEMBLER):
    native_Disassembler()
    return 0
  else:
    return 1
...
proc addTargetData*(td: TargetDataRef; pm: PassManagerRef) {.
    importc: "LLVMAddTargetData", dynlib: LLVMLib.}
